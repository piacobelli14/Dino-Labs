...
const processNextAlert = () => {
  ...
  showAlertInternal(nextAlert);
};

const showAlertInternal = ({ title, message, inputs = [], showCancel = false, resolve }) => {
  isAlertShowing = true;

  if (currentAlertContainer) {
    ReactDOM.unmountComponentAtNode(currentAlertContainer);
...
export function showDialog({ title, message, inputs = [], showCancel = false }) {
  return new Promise((resolve) => {
    const alertData = { title, message, inputs, showCancel, resolve };

    if (isAlertShowing) {
      alertQueue.push(alertData);
    } else {
      showAlertInternal(alertData);
    }
  });
}