...
import "../../../styles/mainStyles/DinoLabsMedia/DinoLabsAudioEditor/DinoLabsAudioEditor.css";
...
export default function DinoLabsAudioEditor({ fileHandle }) {
    const DEFAULT_ATTACK = 5.0;
    const DEFAULT_DECAY = 5.0;
    const DEFAULT_SUSTAIN = 0.1;
    const DEFAULT_RELEASE = 5.0;
    const DEFAULT_VOLUME = 100;
    const DEFAULT_PAN = 0;
    const DEFAULT_VOCAL_ISOLATION = 4;
    ...
    const createReverbImpulseResponse = (audioCtx, duration, decay, reverse) => {
        ...
        for (let channel = 0; channel < impulse.numberOfChannels; channel++) {
            ...
            for (let i = 0; i < length; i++) {
                ...
                impulseChannelData[i] = (Math.random() * 2 - 1) * Math.pow(1 - n / length, decay);
            }
        }
        return impulse;
    };
    ...
    const [vocalsCollapsed, setVocalsCollapsed] = useState(true); 
    const [adsrCollapsed, setADSRCollapsed] = useState(true); 
    ...
    const [vocalIsolation, setVocalIsolation] = useState(DEFAULT_VOCAL_ISOLATION);
    ...
    const [sustain, setSustain] = useState(DEFAULT_SUSTAIN);
    ...
    const [fftSize, setFftSize] = useState(DEFAULT_FFT_SIZE);
    ...
    const TimelineClip = ({ clip, index, trackIndex, isSelected, onSelect, onSplit, onDelete, timelineScale, timelineOffset, trackHeight, dropTargetIndex, draggedClipIndex, onDragStart, onDragOver, onDrop, onClipResize }) => {
        ...
        return (
            ...
            >
                ...
                <div className="trim-handle left" style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: "4px", backgroundColor: "rgba(92, 43, 226, 0.8)", cursor: "ew-resize", opacity: isSelected ? 1 : 0 }} onMouseDown={(e) => handleTrimMouseDown(e, 'left')} />
                <div className="trim-handle right" style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "4px", backgroundColor: "rgba(92, 43, 226, 0.8)", cursor: "ew-resize", opacity: isSelected ? 1 : 0 }} onMouseDown={(e) => handleTrimMouseDown(e, 'right')} />
            </div>
        );
    };
    ...
    const Timeline = ({ clips, selectedClips, onClipSelect, onClipSplit, onClipDelete, onClipMove, currentTime, duration, onTimelineSeek, timelineZoom, onTimelineZoom, multiSelectMode, setMultiSelectMode, onClipResize }) => {
        ...
        const handleClipDrop = (e, clipIndex, trackIndex) => {
            e.preventDefault();
            if (draggedClipIndex !== null && draggedTrackIndex !== null && !(draggedClipIndex === clipIndex && draggedTrackIndex === trackIndex)) {
            ...
            <div className="dinolabsVideoInputBottomBarTimelineSupplement">
                <div className="dinolabsVideoEditorTimelineHeader">
                    ...
                    <div className="dinolabsVideoEditorTimelineHeaderSupplementTrailing">
                        <span>{Math.floor(currentTime / 60)}:{Math.floor(currentTime % 60).toString().padStart(2, "0")} / {Math.floor(duration / 60)}:{Math.floor(duration % 60).toString().padStart(2, "0")}</span>
                    </div>
                </div>
                <div className="dinolabsVideoEditorTimeline" ref={timelineRef} onClick={handleTimelineClick} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop} onScroll={handleScroll} style={{ backgroundColor: isDraggingOver ? "rgba(92, 43, 226, 0.2)" : "", overflowX: "auto", overflowY: "auto", maxHeight: "400px" }}>
                        ...
                        <div className="dinolabsVideoEditorTimelineTrackContainer">
                            {tracks.map((track, trackIndex) => (
                                <div key={trackIndex} className="dinolabsVideoEditorTimelineVideoTracks">
                                    ...
                                    <div className="dinolabsVideoEditorTimelineVideoTrackContent" style={{ height: trackHeight, position: "relative", minWidth: timelineWidth }}>
    ...
    const releaseEnvelope = () => {
        ...
        const now = audioCtx.currentTime;
        envelopeGainNode.gain.cancelScheduledValues(now);
        const currentVal = envelopeGainNode.gain.value < 0.0001 ? 0.0001 : envelopeGainNode.gain.value;
    ...
    const handleMergeTracks = async () => {
        ...
        envelopeGainNode.gain.exponentialRampToValueAtTime(Math.max(sustain, 0.0001), attack + decay);
        const totalDur = duration;
        const fadeStart = totalDur - release;
        if (fadeStart > attack + decay) {
            envelopeGainNode.gain.setValueAtTime(Math.max(sustain, 0.0001), fadeStart);
            envelopeGainNode.gain.exponentialRampToValueAtTime(0.0001, fadeStart + release);
        }
    ...
    const handleResetAll = () => {
        ...
        setTreble(0);
        setVocalBoost(0);
        setVocalIsolation(DEFAULT_VOCAL_ISOLATION);
        setEcho(0);
        setReverb(0);
        setPitchShift(0);
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const offlineCtx = new (window.OfflineAudioContext || window.webkitOfflineAudioContext)(1, 44100 * 40, 44100);
    ...
    useEffect(() => {
        ...
        const drawLeftMeter = () => {
            ...
            const activeBarsLeft = Math.floor(rmsLeft * TOTAL_BARS);
            ctx.clearRect(0, 0, rect.width, rect.height);
            
            const barHeight = meterHeight / TOTAL_BARS;
    ...
    useEffect(() => {
        ...
        const drawRightMeter = () => {
            ...
            const activeBarsRight = Math.floor(rmsRight * TOTAL_BARS);
            ctx.clearRect(0, 0, rect.width, rect.height);
    ...
    return (
        <div className="dinolabsAudioEditorWrapper">
            <div className="dinolabsAudioEditorToolbar">
                <div className="dinolabsAudioEditorHeaderFlexDark" style={{ marginBottom: 0, border: "none" }}>
                    <label className="dinolabsAudioEditorCellTitle">
                    <FontAwesomeIcon icon={faPenToSquare} /> Edit
                    </label>
                </div>
                <div className={!audioCollapsed ? "dinolabsAudioEditorCellWrapper" : "dinolabsAudioEditorCellWrapperCollapsed"}>
                            ...
                            {!audioCollapsed && (
                                <>
                                    ...
                                    <Tippy content="Reset All" theme="tooltip-light">
                                        <button onClick={handleResetAll} className="dinolabsAudioEditorToolButtonHeader">
                             ...
                             <Tippy content="Collapse Audio" theme="tooltip-light">
                                <button onClick={() => { setAudioCollapsed(!audioCollapsed) }} className="dinolabsVideoEditorToolButtonHeader">
                                    <FontAwesomeIcon icon={audioCollapsed ? faChevronRight : faChevronDown} />
                                </button>
                            </Tippy>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setVolume((prev) => Math.max(prev - 10, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="0" max="100" value={volume} onChange={(e) => setVolume(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setVolume((prev) => Math.min(prev + 10, 100))} className="dinolabsAudioEditorToolButton">
                    ...
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setPan((prev) => Math.max(prev - 0.1, -1))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="-1" max="1" step="0.1" value={pan} onChange={(e) => setPan(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setPan((prev) => Math.min(prev + 0.1, 1))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                ...
                <div className={!pitchCollapsed ? "dinolabsAudioEditorCellWrapper" : "dinolabsAudioEditorCellWrapperCollapsed"}>
                    <div className="dinolabsAudioEditorHeaderFlex">
                        <label className="dinolabsAudioEditorCellTitle">
                        ...
                        <div className="dinolabsAudioEditorCellFlexSupplement">
                            {!pitchCollapsed && (
                                ...
                                <button onClick={handleResetPitch} className="dinolabsAudioEditorToolButtonHeader">
                                    <FontAwesomeIcon icon={faArrowsRotate} />
                                </button>
                            ...
                            <Tippy content="Collapse Pitch" theme="tooltip-light">
                                <button onClick={() => { setPitchCollapsed(!pitchCollapsed) }} className="dinolabsVideoEditorToolButtonHeader">
                                    <FontAwesomeIcon icon={pitchCollapsed ? faChevronRight : faChevronDown} />
                                </button>
                            </Tippy>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setPitchShift((prev) => Math.max(prev - 1, -12))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="-12" max="12" value={pitchShift} onChange={(e) => setPitchShift(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setPitchShift((prev) => Math.min(prev + 1, 12))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                ...
                <div className={!eqCollapsed ? "dinolabsAudioEditorCellWrapper" : "dinolabsAudioEditorCellWrapperCollapsed"}>
                    <div className="dinolabsAudioEditorHeaderFlex">
                        <label className="dinolabsAudioEditorCellTitle">
                        ...
                        <div className="dinolabsAudioEditorCellFlexSupplement">
                            {!eqCollapsed && (
                                <Tippy content="Reset EQ" theme="tooltip-light">
                                    <button onClick={handleResetEQ} className="dinolabsAudioEditorToolButtonHeader">
                                        <FontAwesomeIcon icon={faArrowsRotate} />
                                    </button>
                                </Tippy>
                            )}

                            <Tippy content="Collapse EQ" theme="tooltip-light">
                                <button onClick={() => { setEQCollapsed(!eqCollapsed) }} className="dinolabsVideoEditorToolButtonHeader">
                                    <FontAwesomeIcon icon={eqCollapsed ? faChevronRight : faChevronDown} />
                                </button>
                            </Tippy>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setBass((prev) => Math.max(prev - 1, -30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="-30" max="30" value={bass} onChange={(e) => setBass(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setBass((prev) => Math.min(prev + 1, 30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setMid((prev) => Math.max(prev - 1, -30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="-30" max="30" value={mid} onChange={(e) => setMid(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setMid((prev) => Math.min(prev + 1, 30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setTreble((prev) => Math.max(prev - 1, -30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="-30" max="30" value={treble} onChange={(e) => setTreble(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setTreble((prev) => Math.min(prev + 1, 30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                ...
                <div className={!effectsCollapsed ? "dinolabsAudioEditorCellWrapper" : "dinolabsAudioEditorCellWrapperCollapsed"}>
                    <div className="dinolabsAudioEditorHeaderFlex">
                        <label className="dinolabsAudioEditorCellTitle">
                        ...
                        <div className="dinolabsAudioEditorCellFlexSupplement">
                            {!effectsCollapsed && (
                                <Tippy content="Reset Effects" theme="tooltip-light">
                                    <button onClick={handleResetEffects} className="dinolabsAudioEditorToolButtonHeader">
                                        <FontAwesomeIcon icon={faArrowsRotate} />
                                    </button>
                                </Tippy>
                            )}

                            <Tippy content="Collapse Effects" theme="tooltip-light">
                                <button onClick={() => { setEffectsCollapsed(!effectsCollapsed) }} className="dinolabsVideoEditorToolButtonHeader">
                                    <FontAwesomeIcon icon={effectsCollapsed ? faChevronRight : faChevronDown} />
                                </button>
                            </Tippy>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setEcho((prev) => Math.max(prev - 5, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="0" max="100" value={echo} onChange={(e) => setEcho(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setEcho((prev) => Math.min(prev + 5, 100))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setReverb((prev) => Math.max(prev - 5, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="0" max="100" value={reverb} onChange={(e) => setReverb(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setReverb((prev) => Math.min(prev + 5, 100))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setFadeIn((prev) => Math.max(prev - 0.5, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="0" max="10" step="0.5" value={fadeIn} onChange={(e) => setFadeIn(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setFadeIn((prev) => Math.min(prev + 0.5, 10))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setFadeOut((prev) => Math.max(prev - 0.5, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="0" max="10" step="0.5" value={fadeOut} onChange={(e) => setFadeOut(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setFadeOut((prev) => Math.min(prev + 0.5, 10))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                ...
                <div className={!vocalsCollapsed ? "dinolabsAudioEditorCellWrapper" : "dinolabsAudioEditorCellWrapperCollapsed"}>
                    <div className="dinolabsAudioEditorHeaderFlex">
                        <label className="dinolabsAudioEditorCellTitle">
                        ...
                        <div className="dinolabsAudioEditorCellFlexSupplement">
                            {!vocalsCollapsed && (
                                <Tippy content="Reset Vocals" theme="tooltip-light">
                                    <button onClick={handleResetVocals} className="dinolabsAudioEditorToolButtonHeader">
                                        <FontAwesomeIcon icon={faArrowsRotate} />
                                    </button>
                                </Tippy>
                            )}

                            <Tippy content="Collapse Vocals" theme="tooltip-light">
                                <button onClick={() => { setVocalsCollapsed(!vocalsCollapsed) }} className="dinolabsVideoEditorToolButtonHeader">
                                    <FontAwesomeIcon icon={vocalsCollapsed ? faChevronRight : faChevronDown} />
                                </button>
                            </Tippy>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setVocalBoost((prev) => Math.max(prev - 1, -30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" min="-30" max="30" value={vocalBoost} onChange={(e) => setVocalBoost(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setVocalBoost((prev) => Math.min(prev + 1, 30))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setVocalIsolation((prev) => Math.max(prev - 0.1, 0.5))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" step="0.1" min="0.5" max="20" value={vocalIsolation} onChange={(e) => setVocalIsolation(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setVocalIsolation((prev) => Math.min(prev + 0.1, 20))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                ...
                <div className={!adsrCollapsed ? "dinolabsAudioEditorCellWrapper" : "dinolabsAudioEditorCellWrapperCollapsed"}>
                    <div className="dinolabsAudioEditorHeaderFlex">
                        <label className="dinolabsAudioEditorCellTitle">
                        ...
                        <div className="dinolabsAudioEditorCellFlexSupplement">
                            {!adsrCollapsed && (
                                <Tippy content="Reset ADSR" theme="tooltip-light">
                                    <button onClick={handleResetADSR} className="dinolabsAudioEditorToolButtonHeader">
                                        <FontAwesomeIcon icon={faArrowsRotate} />
                                    </button>
                                </Tippy>
                            )}

                            <Tippy content="Collapse ADSR" theme="tooltip-light">
                                <button onClick={() => { setADSRCollapsed(!adsrCollapsed) }} className="dinolabsVideoEditorToolButtonHeader">
                                    <FontAwesomeIcon icon={adsrCollapsed ? faChevronRight : faChevronDown} />
                                </button>
                            </Tippy>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setAttack((prev) => Math.max(prev - 1, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" step="1" min="0" max="10" value={attack} onChange={(e) => setAttack(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setAttack((prev) => Math.min(prev + 1, 10))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setDecay((prev) => Math.max(prev - 1, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" step="1" min="0" max="10" value={decay} onChange={(e) => setDecay(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setDecay((prev) => Math.min(prev + 1, 10))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setSustain((prev) => Math.max(prev - 0.1, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" step="0.1" min="0" max="1" value={sustain} onChange={(e) => setSustain(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setSustain((prev) => Math.min(prev + 0.1, 1))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
                    <div className="dinolabsAudioEditorCellFlexStack">
                        ...
                        <div className="dinolabsAudioEditorCellFlex">
                            <button onClick={() => setRelease((prev) => Math.max(prev - 1, 0))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faMinus} />
                            </button>
                            <div className="dinolabsAudioEditorSliderWrapper">
                                <input type="range" step="1" min="0" max="10" value={release} onChange={(e) => setRelease(Number(e.target.value))} className="dinolabsSettingsSlider" />
                            </div>
                            <button onClick={() => setRelease((prev) => Math.min(prev + 1, 10))} className="dinolabsAudioEditorToolButton">
                                <FontAwesomeIcon icon={faPlus} />
                            </button>
                        </div>
                    </div>
            ...
            <div className="dinolabsAudioEditorContainerWrapper">
                    ...
                    <div className="dinolabsAudioEditorPlaybackControls">
                        {[0.5, 1.0, 1.25, 1.5, 2.0].map((rate) => (
                            <Tippy key={rate} content={`${rate}x Playback`} theme="tooltip-light">
                                <button className="dinolabsAudioButtonX" onClick={() => setPlaybackRateHandler(rate)} style={{ color: currentPlaybackRate === rate ? "#5C2BE2" : "" }}>
                                    {rate}x
                                </button>
                            </Tippy>
                        ))}
                ...
                <div className="dinolabsAudioEditorContainer" style={{ height: showTimeline ? "40%" : "80%", minHeight: showTimeline ? "40%" : "80%", maxHeight: showTimeline ? "40%" : "80%", overflow: "hidden" }}>
                    <div className="dinolabsAudioEditorInnerWrapper">
                        <div className="dinolabsAudioEditorInnerWrapperWaveForm">
                            <canvas ref={waveformCanvasRef} style={{ width: "100%", height: "100%", display: "block", cursor: multiSelectMode ? "crosshair" : "pointer" }} onMouseDown={handleWaveformMouseDown} onMouseMove={handleWaveformMouseMove} onMouseUp={handleWaveformMouseUp} />
                        </div>
                        <div className="dinolabsAudioEditorInnerWrapperSupplementsFlex">
                            <div className="dinolabsAudioEditorInnerWrapperSupplementBig">
                                <canvas ref={frequencyBarsCanvasRef} style={{ width: "100%", height: "100%", display: "block" }} />
                            </div>
                            <div className="dinolabsAudioEditorInnerWrapperSupplementBig">
                                <canvas ref={oscilloscopeCanvasRef} style={{ width: "100%", height: "100%", display: "block" }} />
                            </div>
                            <div className="dinolabsAudioEditorInnerWrapperSupplementSmall">
                                <canvas ref={stereoLeftCanvasRef} style={{ width: "100%", height: "100%", display: "block" }} />
                            </div>
                            <div className="dinolabsAudioEditorInnerWrapperSupplementSmall">
                                <canvas ref={stereoRightCanvasRef} style={{ width: "100%", height: "100%", display: "block" }} />
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="dinolabsAudioInputBottomBar">
                    <div className="dinolabsAudioContentFlexBig">
                        <Tippy content="Rewind 5 Seconds" theme="tooltip-light">
                            <button onClick={() => skipBackward(5)} className="dinolabsAudioButtonSupplementLeading">
                                <FontAwesomeIcon icon={faBackward} />
                            </button>
                        </Tippy>
                        <Tippy content={isPlaying ? "Pause" : "Play"} theme="tooltip-light">
                            <button onClick={togglePlay} className="dinolabsAudioButton">
                                <FontAwesomeIcon icon={isPlaying ? faStop : faPlay} />
                            </button>
                        </Tippy>
                        <Tippy content="Loop" theme="tooltip-light">
                            <button onClick={toggleLoop} className="dinolabsAudioButton" style={{ color: isLooping ? "#5C2BE2" : "" }}>
                                <FontAwesomeIcon icon={faRepeat} />
                            </button>
                        </Tippy>
                        <Tippy content="Skip 5 Seconds" theme="tooltip-light">
                            <button onClick={() => skipForward(5)} className="dinolabsAudioButtonSupplementTrailing">
