...
const DinoLabsPluginsColorTypeLab = () => {
  ...
  const [fontFamily, setFontFamily] = useState("InterVariable, Inter, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif");
  ...
  const extractColorsFromImage = (imageData) => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      const rBucket = Math.floor(r / 5) * 5;
      const gBucket = Math.floor(g / 5) * 5;
      const bBucket = Math.floor(b / 5) * 5;
      const key = `${rBucket},${gBucket},${bBucket}`;
      
      if (!colorMap.has(key)) {
        colorMap.set(key, { r: rBucket, g: gBucket, b: bBucket, count: 0 });
      }
    ...
    colors.forEach(c => {
      ...
      c.saturation = max === 0 ? 0 : (max - min) / max;
    });
    ...
    selectedColors.sort((a, b) => {
      if (a.saturation > 0.15 && b.saturation <= 0.15) return -1;
      if (a.saturation <= 0.15 && b.saturation > 0.15) return 1;
      return b.count - a.count;
    });

    return selectedColors.map(c => rgbToHex({ r: c.r, g: c.g, b: c.b }));
  };

  const processUploadedImage = (file) => {
    if (!file || !file.type.startsWith("image/")) {
      setExtractionStatus("Please upload a valid image file.");
      return;
    }
    ...
    img.onload = () => {
      ...
      try {
        ...
        if (colors.length > 0) {
          ...
          setExtractionStatus(`Extracted ${colors.length} colors successfully.`);
        } else {
          setExtractionStatus("No colors could be extracted from this image.");
        }
      } catch (error) {
        setExtractionStatus("Error processing image. Please try another file.");
      }
    };

    img.onerror = () => {
      setExtractionStatus("Error loading image. Please try another file.");
    };
  ...
  const applyExtractedColors = () => {
    if (extractedColors.length < 3) {
      setExtractionStatus("Need at least 3 colors to apply.");
      return;
    }

    setBrandPrimary(extractedColors[0]);
    setBrandSecondary(extractedColors[1]);
    setBrandAccent(extractedColors[2]);
    
    if (extractedColors.length >= 5) {
      setDvLeft(extractedColors[3]);
      setDvRight(extractedColors[4]);
    }
    
    if (extractedColors.length >= 6) {
      setGradFrom(extractedColors[0]);
      setGradTo(extractedColors[5]);
    }

    setExtractionStatus("Colors applied successfully.");
  };
        ...
        <aside className="dinolabsColorTypeLabSidebar">
          ...
          <div className="dinolabsColorTypeLabSection">
            <div className="dinolabsColorTypeLabSectionTitle">
              ...
              <span>Image Color Extraction</span>
            </div>

            <div className="dinolabsColorTypeLabRow">
              <label className="dinolabsColorTypeLabLabel">Upload Image</label>
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
            ...
            {extractionStatus && (
              <div className="dinolabsColorTypeLabRow">
                <div className="dinolabsColorTypeLabSmall" style={{ padding: "8px" }}>
                  {extractionStatus}
                </div>
              </div>
            )}
            ...
            {extractedColors.length > 0 && (
              <>
                <div className="dinolabsColorTypeLabSwatchRow">
                  {extractedColors.map((color, index) => (
                    <button
                      ...
                      style={{ 
                        background: color, 
                        color: luminance(color) > 0.45 ? "#0f172a" : "#f8fafc",
                        minHeight: "40px"
                      }}
                ...
                <div className="dinolabsColorTypeLabRow dinolabsColorTypeLabActions">
                  <button className="dinolabsColorTypeLabBtn" onClick={applyExtractedColors}>
                    <FontAwesomeIcon icon={faPalette} /> Apply Colors
                  </button>
                </div>
              </>
            )}
          ...
          <div className="dinolabsColorTypeLabSection">
            ...
            <DinoColorField 
              label="Base" 
              value={harmonyBase} 
              onChange={setHarmonyBase} 
              ariaLabel="Harmony Base Color." 
            />
